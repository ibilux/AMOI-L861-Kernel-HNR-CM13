...
//tangjx Modify 20150601 create proc entry at  /proc/lcm_name
#include <linux/proc_fs.h>
static ssize_t mtkfb_amoi_lcm_name_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *p = page;
    int len = 0; 
...
//tangjx Modify 20150601 create proc entry at  /proc/lcm_name
//amoibsp luoyh@20150208 for amoi eng test -begin
static int mt_cmdinfo_show(struct seq_file *m, void *v)
{
	char strChipIDShow[50]={0};
	char* lcm_name = mtkfb_find_lcm_driver();
   if(NULL != lcm_name) {
   		strcpy(strChipIDShow,lcm_name);
    }
    else{
        strcpy(strChipIDShow,"unknow");
    }
...
//amoibsp luoyh@20150208 for amoi eng test - end
 //amoibsp luoyh@20150127 beging
 static int amoi_lcm_name_proc_write(struct file *file, const char *buffer, size_t count, loff_t *ppos) {
	 return 1;
 }
 
 static const struct file_operations amoi_lcm_name = { 
 	 //.owner = THIS_MODULE,
	 .write = amoi_lcm_name_proc_write,
	 .open = mt_cmdinfo_open_proc,
	 .read = seq_read,
 };
 //amoibsp luoyh@20150127 end
...
//tangjx Modify 20150601 create proc entry at  /proc/lcm_name
    proc_create("lcm_name", S_IRUGO, NULL, &amoi_lcm_name);
//tangjx Modify 20150601 create proc entry at  /proc/lcm_name
